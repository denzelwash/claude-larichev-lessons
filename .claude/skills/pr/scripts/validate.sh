...
# --- Проверка 1: ветка отслеживает remote ---
...
if ! git rev-parse @{u} &>/dev/null; then
  ...
  if [ "$BEHIND" -gt 0 ]; then
    ...
    echo "    OK — ветка запушена в remote."
  fi
fi
...
DIRTY_FILES=$(git diff "origin/$BASE"...HEAD --name-only 2>/dev/null | grep -E '(^|/)\.env$|(^|/)\.env\.|/dist/|/build/|\.sqlite$|\.dump$|\.sql$' || true)
